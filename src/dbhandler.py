...
class Dbhandler:
    ...
    def add_user(self, uid: int, name: str, permlev: int):
        self.cursor.execute('''INSERT INTO users(uid, permlevel, name) VALUES(?,?,?)'''(uid, permlev, name))
        self.conn.commit()
        return 0
    ...
    def increment_user_message_count(self, author_uid: int, name: str, mention):
        ...
        if (author_uid in uid_dict.keys()):
            ...
            self.cursor.execute(
                f'''UPDATE users SET msgcount={uid_dict[author_uid][0] + 1} WHERE uid='{author_uid}' ''')
        else:
            self.cursor.execute(f'''INSERT INTO users(uid,permlevel,name,msgcount,readable_name) VALUES(?,?,?,?,?)''',
    ...
    def set_perm(self, user, newpermlev):
        ...
        exists = len(res) != 0
        if (exists):
            self.cursor.execute(f'''UPDATE users SET permlevel = ? WHERE uid==? ''', (newpermlev, uid))
        else:
    ...
    def nhentai_block(self, id, noswitch=False) -> None:
        ...
        if len(res) > 0:
            curr_state = res[0][0]
            if noswitch:
                self.cursor.execute('''UPDATE nhentai SET blocked=1 WHERE id=?''', (id,))
            else:
                self.cursor.execute('''UPDATE nhentai SET blocked=? WHERE id=?''', (1 - int(curr_state), id))
        else:
    ...
    def add_meme_template(self, template_name, template_id):
        self.cursor.execute('''INSERT INTO meme_templates(template_name,template_id) VALUES(?,?)''',
    ...
    def get_avg_guess(self, ownuid) -> int:
        self.cursor.execute("SELECT avg(guess) FROM guesses WHERE uid!=?",
                            (ownuid,))  # ignore guess of the user calling the cmd
        res = self.cursor.fetchall()
