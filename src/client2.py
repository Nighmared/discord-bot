        ...
        while self.msgq.not_empty:
            ...
            if next_inp.startswith(PREFIX):  # client command
                ...
                if cmd == "channels":
                    ...
                    self.chosen_channel = (await self.server.fetch_channels())[
                        int(args[0])
                    ]
                elif cmd == "setserver":
                    ...
                    self.coutwin.refresh()
                    self.coutwin.addstr(
                        0, 0, f"Set server to {self.server.name}", curses.A_BLINK
                    )
                    self.coutwin.refresh()
                elif cmd == "quit":
                    stdscr.clear()
...
def curses_get_input(
    stdscr: curses.window, inputwin: curses.window, msgqueue: Queue, status
):
    to_send = ""
    next_char_x = 2
    next_char_y = 0
    ...
    while status.run:
        ...
        if curr_in == "\n":
            ...
            curr_in == "KEY_BACKSPACE"
            or curr_in == curses.KEY_BACKSPACE
            or curr_in == "^?"
            or curr_in == "\b"
            or curr_in == "\x7f"
        ):
...
reader_thread = threading.Thread(
    target=curses_get_input, args=(stdscr, inpwin, msgq, status)
)
reader_thread.start()
